...
Cumo と PyTorch を同じバッチでインターリーブした区間別。s/step (1 ステップにかかった秒数) で、小さいほど速い。本/step は 1 ステップで GPU に投入するカーネルの本数。Cumo / PyTorch は時間の比なので、ほかの表と逆に 1 を超えるほど Cumo が遅く、後ろの n/10 も Cumo が遅かった回数 (11 ラウンドの先頭を位置で捨てて 10 ラウンド、対照つき)。
