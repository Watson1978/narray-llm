...
|---|---|---|---|---|---|---|
| 有り | 64 | 812.4 | 230.5 | 3.53 | 795.2 | 0.977 |
| 有り | 256 | 777.0 | 228.2 | 3.41 | 709.0 | 0.913 |
| 無し | 64 | 472.6 | 208.8 | 2.27 | 494.3 | 1.049 |
| 無し | 256 | 216.2 | 142.3 | 1.52 | 241.4 | 1.117 |

バッチ生成 (KV キャッシュ有り、長さ 256、tokens/sec は全系列の合計)。

| batch | Cumo | CuPy | PyTorch | Cumo / CuPy | PyTorch / Cumo |
|---|---|---|---|---|---|
| 1 | 785.9 | 225.2 | 708.0 | 3.50 10/10 | 0.900 0/10 |
| 8 | 3824.4 | 1712.2 | 3613.3 | 2.23 10/10 | 0.942 0/10 |
...
| KV キャッシュ | 生成長 | Cumo | CuPy | Cumo / CuPy | PyTorch eager | PyTorch / Cumo |
|---|---|---|---|---|---|---|
| 有り | 64 | 587.1 | 216.7 | 2.79 | 537.3 | 0.891 |
| 有り | 200 | 582.4 | 215.7 | 2.70 | 531.4 | 0.910 |
| 無し | 64 | 465.7 | 203.0 | 2.29 | 456.0 | 0.976 |
| 無し | 200 | 253.4 | 172.9 | 1.47 | 263.9 | 1.045 |
...
|---|---|---|---|---|---|
| 64 | 465.5 | 150.5 | 359.1 | 3.09 | 0.774 |
| 200 | 501.7 | 147.8 | 371.3 | 3.43 | 0.740 |

1 トークン 629 本の時点の表。経過と内訳は [mamba-130m.md](docs/results/mamba-130m.md)。

### Switch Transformer base-8 (MoE)

| 条件 | Cumo | CuPy | PyTorch | Cumo / CuPy | PyTorch / Cumo |
|---|---|---|---|---|---|
| decode 72 トークン | 155.8 | 50.1 | 144.9 | 3.13 | 0.925 |
| encode 2048 トークン | 2831.1 | 2732.2 | 2810.5 | またぐ | またぐ |
...
| 条件 | Cumo | CuPy | PyTorch | Cumo / CuPy | PyTorch / Cumo |
|---|---|---|---|---|---|
| decode (tokens/sec) | 756.1 | 231.9 | 716.7 | 3.27 10/10 | 0.945 0/10 |
| encode (positions/sec) | 228,269 | 179,117 | 262,905 | 1.27 10/10 | 1.151 10/10 |
...
|---|---|---|---|---|
| `shift` | 625.0 | 430.2 | 1.45 10/10 | 526 |
| `unfold` | 922.1 | 993.8 | 0.93 0/10 | 330 |
| 綴りの効き (`unfold` / `shift`) | 1.48 倍 | 2.31 倍 | | |
| 対照 (Cumo `unfold` をもう一度) | 923.0 | | またぐ | |

cuDNN の腕は別のバッチなので、上の表とまたいで比を取らないこと。CuPy 14.2 は cuDNN の畳み込みを公開していないので、この表の相手は PyTorch。
...
関門は llm.c の `test_gpt2.c` の許容誤差 (勾配 2e-2、損失 1e-2)。
...
Cumo と PyTorch を同じバッチでインターリーブした区間別 (s/step、11 ラウンドの先頭を位置で捨てて 10 ラウンド、対照つき)。
...
decode で GPU に投入するカーネルの本数 (nsys、`--length 128` と `--length 64` の差 ÷ 64)。
